...
### Added
- ✨ 生词高亮控制：设置面板新增「生词高亮控制」开关，可一键开关笔记内的生词高亮，切换后立即刷新编辑视图与阅读视图

### Changed
- 🔒 API Key 与欧路 Token 改为存入系统钥匙串（macOS 钥匙串 / Windows 凭据管理器），`data.json` 中不再保存明文密钥
- 🔒 密钥输入框改为密码型显示，并关闭浏览器自动填充与拼写检查
- 🔇 调试日志统一走内部通道，默认完全关闭，不再向控制台输出查词内容、生词本词条或选中文本

### Fixed
- 🐛 修复生词高亮开关字段缺失导致全新安装的用户笔记内高亮默认全部关闭的问题（该问题自 1.4.3 起存在）
- 🐛 修复设置面板缺少生词高亮开关、用户无法自行开启的问题
