 ...
 *
 * API keys can live in one of two places, picked by the user through the
 * `secretStorageMode` setting:
 *
 ...
 * while the system keychain backend is active.
 */
 ...
 * used as the source when the keychain has nothing for that field yet, in
 ...
 * value; if the keychain is unavailable the plain-text value is kept, so
